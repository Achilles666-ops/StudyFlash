    ...
    >
      ...
      <div className="w-full md:w-1/2 bg-white flex flex-col justify-between p-8 md:p-16">
        <div className="my-auto max-w-md w-full mx-auto space-y-8">
          ...
          {(authError || error) && (
            <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-xs font-semibold text-red-600 leading-relaxed text-center">
              {error || authError}
            </div>
          )}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-white hover:bg-[#F9FAFB] text-[#1A1A1A] rounded-2xl border border-[#E5E7EB] font-bold text-sm shadow-sm hover:shadow-md hover:scale-[1.01] transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <div className="w-5 h-5 border-2 border-gray-300 border-t-[#0F7B6C] rounded-full animate-spin shrink-0" />
            ) : (
